...
import { anadirJugadores } from "./dbFunction";

export function AnadirJugador({modifiedTable,setModifiedTable}){
    const [datos,setDatos] = useState({
     ...
     useEffect(()=>{
        if(datos.edad > 0){
            const response = anadirJugadores(datos);
            response.then((data)=>{
                console.log(data)
            })
    ...
    return(
        ...
        <form>
            ...
            <button onClick={(e)=>{
                setModifiedTable(modifiedTable + 1)
                e.preventDefault()
                setDatos({
